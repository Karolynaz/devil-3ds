...
void UseBeltSlot(size_t slotIndex)
{
	if (slotIndex >= MaxBeltItems)
		return;
	if (MyPlayer == nullptr || (MyPlayer->_pInvincible && MyPlayer->hasNoLife()))
		return;
...
void TriggerPowerSlot(size_t slotIndex)
{
	if (MyPlayer == nullptr)
		return;

	if (SpellSelectFlag) {
		SetSpeedSpell(slotIndex);
		return;
	}

	if (slotIndex < NumHotkeys && MyPlayer->_pSplHotKey[slotIndex] != SpellID::Invalid) {
		if (!*GetOptions().Gameplay.quickCast)
			ToggleSpell(slotIndex);
		else
			QuickCast(slotIndex);
		return;
	}

	// If slot 0 is not assigned a spell or if player is attacking, fall back to Primary Action
	if (slotIndex == 0) {
		PerformPrimaryAction();
	}
}
